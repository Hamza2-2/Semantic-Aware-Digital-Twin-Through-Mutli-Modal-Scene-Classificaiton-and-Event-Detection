// Audio prediction testing screen
import 'dart:ui';
...
class _AudioTestingScreenState extends State<AudioTestingScreen>
  ...
  List<Map<String, dynamic>> _applyBusBoost(List<dynamic> predictions) {
    if (_classifier.audioModelType != 'passt') {
      return predictions
  ...
  Future<void> _connectToAudioStream() async {
    ...
    try {
      ...
      final request = http.Request('HEAD', checkUri);
  ...
  Future<void> _processAudio() async {
    ...
    try {
      ...
      final modelType = (result['modelType'] as String?) ?? _classifier.audioModelType;
      final eventGate = modelType == 'passt' ? 0.35 : 0.10;
    ...
    try {
      ...
      if (_enableEventDetection) {
        ...
        if (eventResult != null) {
          ...
          final streamEventGate = _classifier.audioModelType == 'passt' ? 0.35 : 0.10;
          if (eventResult.sceneConfidence < streamEventGate) {
            eventResult = null;
            eventType = null;
          }
        } else {
  ...
  Future<EventDetectionResult?> _runEventDetectionForFile(
    ...
    try {
      final eventResult = await _eventService.detectEventsInAudio(
        _selectedFilePath!,
        confidenceThreshold: 0.005,
      );
    ...
    try {
      ...
      if (_enableEventDetection) {
        ...
        if (eventResult != null) {
          ...
          final micEventGate = _classifier.audioModelType == 'passt' ? 0.35 : 0.10;
          if (eventResult.sceneConfidence < micEventGate) {
            eventResult = null;
            eventType = null;
          }
        } else {
        ...
        child: Column(
          ...
          children: [
            ...
            if (isError)
              ...
              ..._buildConfidenceBasedContent(scheme),
  ...
  List<Widget> _buildConfidenceBasedContent(ColorScheme scheme) {
    ...
    // Use the model type from the server response (more reliable than client-side setting)
    final serverModelType = (_result?['modelType'] as String?)?.toLowerCase() ?? '';
    final bool isPaSST = serverModelType == 'passt';
    // CNN14 threshold: 5.0% (= confidence 0.05). Audio-only testing only.
    // PaSST uses 35%. No impact on multimodal — that is controlled server-side.
    final double noSceneThreshold = isPaSST ? 35.0 : 5.0;

    // Debug: print actual values to help diagnose any issues
    debugPrint('[AudioTest] confPercent=$confPercent, modelType=$serverModelType, '
        'isPaSST=$isPaSST, threshold=$noSceneThreshold');

    if (confPercent < noSceneThreshold) {
    ...
    if (isPaSST && predictedClass == 'bus') {
      final topPreds = (_result?['topPredictions'] as List? ?? []);
      final classes = topPreds
          .map((p) => (p['class'] as String?)?.toLowerCase() ?? '')
          .toSet();
      if (classes.any((c) => c.contains('metro'))) displayConfPercent += 1.75;
      if (classes.contains('tram')) displayConfPercent += 1.0;
    ...
    return [
      ...
      if (_result?['topPredictions'] != null) ...[
        ...
        ..._applyBusBoost(_result!['topPredictions'] as List).map((pred) {
          final predConf = (pred['confidence'] ?? 0) * 100;
          return Padding(
                ...
                Expanded(
                  ...
                  child: ClipRRect(
                    ...
                    child: LinearProgressIndicator(
                      value: (predConf / 100).clamp(0.0, 1.0),
