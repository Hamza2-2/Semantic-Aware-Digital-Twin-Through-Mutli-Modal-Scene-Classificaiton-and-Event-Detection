// Fusion prediction testing screen
import 'dart:io';
import 'dart:ui';
...
class _MultimodalTestingScreenState extends State<MultimodalTestingScreen>
  ...
  Set<String> _getAllResultClasses() {
    final classes = <String>{};
    void addPreds(dynamic preds) {
      if (preds is List) {
        for (final p in preds) {
          final cls = (p['class'] as String?)?.toLowerCase() ?? '';
          if (cls.isNotEmpty) classes.add(cls);
        }
      }
    }
    addPreds(_result?['topPredictions']);
    addPreds(_result?['videoResult']?['topPredictions']);
    addPreds(_result?['audioResult']?['topPredictions']);
    final v = (_result?['videoResult']?['predictedClass'] as String?)?.toLowerCase();
    final a = (_result?['audioResult']?['predictedClass'] as String?)?.toLowerCase();
    if (v != null) classes.add(v);
    if (a != null) classes.add(a);
    return classes;
  }
  ...
  List<Map<String, dynamic>> _applyBusBoost(List<dynamic> predictions) {
    ...
    double boost = 0.0;
    final allClasses = _getAllResultClasses();
    if (allClasses.any((c) => c.contains('metro'))) boost += _busVsMetroBoost;
    if (allClasses.contains('tram')) boost += _busVsTramBoost;
    if (boost > 0) {
      list[busIdx] = Map<String, dynamic>.from(list[busIdx]);
      list[busIdx]['confidence'] =
          ((list[busIdx]['confidence'] as num) + boost).clamp(0.0, 1.0);
    }
    return list;
  }
  ...
  Future<void> _processStream() async {
    ...
    try {
      ...
      final sceneConf = (fusedResult['confidence'] as num?) ?? 0;
      final modelType = (fusedResult['audioResult']?['modelType'] as String?) ?? _classifier.audioModelType;
      final fileEventGate = modelType == 'passt' ? 0.35 : 0.10;
      if (_enableEventDetection &&
          fusedResult['error'] != true &&
          sceneConf >= fileEventGate) {
  ...
  Future<void> _processMultimodal() async {
    ...
    try {
      ...
      final modelType = (result['audioResult']?['modelType'] as String?) ?? _classifier.audioModelType;
      final fileEventGate = modelType == 'passt' ? 0.35 : 0.10;
      if (_enableEventDetection &&
          result['error'] != true &&
          sceneConf >= fileEventGate) {
  ...
  Future<void> _processHardwareFusion() async {
    ...
    try {
      ...
      final sceneConf = (fusedResult['confidence'] as num?) ?? 0;
      final modelType = (fusedResult['audioResult']?['modelType'] as String?) ?? _classifier.audioModelType;
      final hwEventGate = modelType == 'passt' ? 0.35 : 0.10;
      if (_enableEventDetection &&
          fusedResult['error'] != true &&
          sceneConf >= hwEventGate) {
        try {
        ...
        child: Column(
          ...
          children: [
            ...
            if (isError)
              ...
              ..._buildConfidenceBasedContent(
                  scheme, isDark, isFusion, isMultiscene),
              if (((_result?['confidence'] ?? 0) as num) * 100 >= 50) ...[
              ...
              if (((_result?['confidence'] ?? 0) as num) * 100 >= 55 &&
                  _result?['topPredictions'] != null) ...[
                const SizedBox(height: 20),
                Text(
  ...
  // build widget list
  bool get _usingPaSST {
    final audioType = _result?['audioResult']?['modelType'] as String?;
    if (audioType != null) return audioType == 'passt';
    return _classifier.audioModelType == 'passt';
  }

  List<Widget> _buildConfidenceBasedContent(
      ColorScheme scheme, bool isDark, bool isFusion, bool isMultiscene) {
    final confPercent = ((_result?['confidence'] ?? 0) as num) * 100;
    // For fusion results, use a lower threshold (20%) because fused confidence
    // is naturally lower (~35-40%) when video and audio models disagree.
    // Single-modal thresholds (25% PaSST, 50% CNN14) are too high for fusion.
    final resultType = (_result?['type'] as String?) ?? '';
    final bool isFusionResult = resultType.contains('fusion');
    final double noSceneThreshold =
        isFusionResult ? 20.0 : (_usingPaSST ? 25.0 : 50.0);
    final bool noScene = confPercent < noSceneThreshold;
    ...
    if (_usingPaSST && predictedClass == 'bus') {
      final allClasses = _getAllResultClasses();
      if (allClasses.any((c) => c.contains('metro'))) displayConfPercent += 1.75;
      if (allClasses.contains('tram')) displayConfPercent += 1.0;
      displayConfPercent = displayConfPercent.clamp(0.0, 100.0).toDouble();
    }
          ...
          Row(
            children: [
              Icon(
                ...
                size: 16,
                color: agreement ? Colors.green : Colors.deepOrange,
              ),
              const SizedBox(width: 8),
              Text(
                agreement
  ...
  Widget _buildModalityCard(
    ...
    if (_usingPaSST && predictedClass.toString().toLowerCase() == 'bus') {
      final allClasses = _getAllResultClasses();
      if (allClasses.any((c) => c.contains('metro'))) confidence += 1.75;
      if (allClasses.contains('tram')) confidence += 1.0;
