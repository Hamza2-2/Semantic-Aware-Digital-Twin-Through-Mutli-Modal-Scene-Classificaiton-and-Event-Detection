// Video classification model service
import 'dart:io';
...
class VideoClassifierService {
  ...
  String get audioModelType => _audioModelType;
  set audioModelType(String value) => _audioModelType = value;
  ...
  Future<void> loadSettings() async {
    ...
    _audioModelType = prefs.getString('audio_model_type') ?? _audioModelType;
  }
  ...
  Future<Map<String, dynamic>> classifyAudio(String audioPath) async {
    try {
      ...
      request.fields['audio_model'] = _audioModelType;
    ...
    try {
      final response = await http
          .post(
            ...
            headers: {'Content-Type': 'application/json'},
            body: json.encode({'duration_seconds': durationSeconds, 'audio_model': _audioModelType}),
          )
          .timeout(Duration(seconds: durationSeconds + 60));
      if (response.statusCode == 200) {
        return json.decode(response.body);
