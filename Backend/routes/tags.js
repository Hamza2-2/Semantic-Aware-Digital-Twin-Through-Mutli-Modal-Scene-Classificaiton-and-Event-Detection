...
router.get('/', async (req, res) => {
  try {
    ...
    const [tags, total] = await Promise.all([
      VideoTag.find(query).sort('-createdAt').skip(sk).limit(lim).lean(),
      VideoTag.countDocuments(query)
    ]);
...
router.get('/classes', async (req, res) => {
  try {
    const classes = await VideoTag.aggregate([
...
router.get('/stats', async (req, res) => {
  try {
    const [total, primaryCount, classDist, topByConfidence] = await Promise.all([
      ...
      VideoTag.countDocuments({ isPrimary: true }),
      VideoTag.aggregate([
        { $group: { _id: '$className', count: { $sum: 1 } } },
        { $sort: { count: -1 } },
        { $limit: 10 }
      ]),
      VideoTag.find({ isPrimary: true }).sort('-confidence').limit(5).lean()
    ]);
...
router.get('/by-history/:historyId', async (req, res) => {
  try {
    const tags = await VideoTag.find({ historyRef: req.params.historyId }).sort('rank').lean();
...
router.get('/:id', async (req, res) => {
  try {
    const tag = await VideoTag.findById(req.params.id).lean();
    if (!tag) {
      return res.status(404).json({ success: false, error: 'Tag not found' });
    }


    const history = await VideoHistory.findById(tag.historyRef).lean();
...
router.delete('/:id', async (req, res) => {
  try {
    const result = await VideoTag.findByIdAndDelete(req.params.id);
    if (!result) {
      return res.status(404).json({ success: false, error: 'Tag not found' });
    }
...
router.delete('/', async (req, res) => {
  try {
    ...
    const result = await VideoTag.deleteMany(query);
    res.json({
      success: true,
      message: `Deleted ${result.deletedCount} video tags`,
