...
class CNN14(nn.Module):
    """CNN14 model for audio classification using mel-spectrograms"""
...
class AudioVideoSeparator:
    """
    Separates audio and video from a combined video file
    Preprocesses both modalities for model input
    """
    def __init__(
        self,
        num_frames=16,
    ...
    def extract_from_file(self, video_path):
        """
        Extract and preprocess both video and audio from a single file
        
        Args:
            video_path: Path to video file (e.g., 'data/airport.mp4')
            
        Returns:
            video_tensor: Preprocessed video tensor for model input
            audio_tensor: Preprocessed audio tensor (mel-spectrogram) for model input
        """
    ...
    def extract_video(self, video_path):
        ...
        Returns:
            video_tensor: [1, 3, num_frames, H, W] tensor
        """
        ...
        frames_array = np.array(frames[:self.num_frames]).astype(np.float32) / 255.0
        
                                                  
        video_tensor = torch.from_numpy(frames_array).permute(3, 0, 1, 2).float()
    ...
    def _get_silent_audio_tensor(self):
        """Return a tensor representing silence"""
...
def weighted_fusion(video_logits, audio_logits, video_weight=0.5, audio_weight=0.5):
    """Simple weighted average fusion at probability level"""
...
def confidence_based_fusion(video_logits, audio_logits):
    """Dynamic fusion based on prediction confidence - balanced approach"""
    video_probs = torch.softmax(video_logits, dim=1)
    audio_probs = torch.softmax(audio_logits, dim=1)
...
def max_confidence_fusion(video_logits, audio_logits):
    """Take prediction from most confident modality (probability-level)"""
    video_probs = torch.softmax(video_logits, dim=1)
    audio_probs = torch.softmax(audio_logits, dim=1)
...
def predict_fusion(video_path, fusion_method='confidence'):
    """
    Perform fusion prediction on a video file containing both audio and video
    
    Args:
        video_path: Path to video file
        fusion_method: 'weighted', 'confidence', 'max', or 'average'
    ...
    video_tensor, audio_tensor = separator.extract_from_file(video_path)
    ...
    with torch.no_grad():
        ...
        if fusion_method == 'weighted':
            fused_logits = weighted_fusion(video_logits, audio_logits)
            fused_probs = torch.softmax(fused_logits, dim=1)
        elif fusion_method == 'confidence':
            fused_logits = confidence_based_fusion(video_logits, audio_logits)
            fused_probs = torch.softmax(fused_logits, dim=1)
        elif fusion_method == 'max':
            fused_logits = max_confidence_fusion(video_logits, audio_logits)
            fused_probs = torch.softmax(fused_logits, dim=1)
        elif fusion_method == 'average':
            fused_probs = average_probs_fusion(video_logits, audio_logits)
        else:
...
def get_event_detector(confidence_threshold=0.005):
    """Get or create a singleton AVSlowFast detector instance."""
...
def load_audio_model():
    """Load the CNN14 audio classification model"""
...
def normalize_stream_url(stream_url):
    """Normalize manual camera URLs so common IP camera inputs work reliably."""
...
def capture_stream_to_temp_video(stream_url, duration_seconds, suffix='.mp4'):
    """Capture a live stream into a temp video file and return capture metadata."""
...
def load_and_preprocess_audio(audio_path, sr=44100, duration=10.0, n_mels=128):
    ...
    Args:
        ...
        sr: Sample rate (44100 Hz for DCASE dataset)
        duration: Audio duration in seconds (10s for DCASE)
        n_mels: Number of mel bands (128 for training)
...
def predict_audio(audio_path):
    ...
    Returns:
        dict: Prediction results with class, confidence, and top predictions
    """
    global audio_model, device
...
def predict_fusion_endpoint():
    """
    Perform audio-video fusion prediction on a single video file.
    
    This endpoint extracts both video frames and audio from the uploaded video,
    runs both the video model (R2+1D) and audio model (CNN14) separately,
    then fuses the predictions using the specified fusion method.
    
    Request:
        - video: Video file (multipart/form-data)
        - fusion_method: (optional) 'weighted', 'confidence', 'max', or 'average'
    
    Returns:
        JSON with fused prediction, individual video/audio results, and analysis
    """
    global audio_model
    ...
    try:
        ...
        result = predict_fusion(tmp_path, fusion_method=fusion_method)
...
def predict_fusion_stream_endpoint():
    """
    Perform audio-video fusion prediction from IP stream(s).
    
    For DroidCam/IP Webcam, video is captured from /video (MJPEG) and
    audio from /audio.wav separately, then both are classified and fused.
    
    Request JSON:
        {
            "video_url": "http://192.168.1.x:4747/video",
            "audio_url": "http://192.168.1.x:4747/audio.wav",  (optional, auto-derived)
            "duration": 10,
            "fusion_method": "confidence"
        }
    """
...
def normalize_audio_stream_url(stream_url):
    """Normalize a stream URL into a primary audio candidate.

    The extractor will probe additional candidates for known camera types,
    so this function only establishes a sane first URL.
    """
    normalized = (stream_url or '').strip()
    if not normalized:
        return normalized
...
def get_audio_stream_candidates(stream_url):
    """Return ordered audio URL candidates for the given stream."""
...
def extract_audio_from_stream(stream_url, duration_seconds=10):
    """
    Extract audio from an IP video/audio stream (e.g., IP Webcam, DroidCam, RTSP)
    Uses ffmpeg to capture and convert to WAV.
    
    RECOMMENDED: Use IP Webcam app (port 8080) for reliable audio streaming.
    DroidCam (port 4747) has inconsistent audio support across versions.
    
    Args:
        stream_url: URL of the video/audio stream
        duration_seconds: Duration to capture (default 10s for audio model)
    
    Returns:
        audio_path: Path to temporary WAV file
    """
...
def predict_audio_stream(stream_url, duration_seconds=10):
    """
    Extract audio from IP stream and classify it
    
    Args:
        stream_url: URL of the video/audio stream (DroidCam, RTSP, etc.)
        duration_seconds: Duration to capture (default 10s)
...
def predict_audio_stream_endpoint():
    """
    Classify audio from an IP video/audio stream (DroidCam, RTSP, etc.)
    
    Request JSON:
        {
            "stream_url": "http://192.168.1.x:4747/video",
            "duration_seconds": 10
        }
    """
    global audio_model
...
def detect_events_audio_stream_endpoint():
    """
    Event detection from audio IP stream using CNN14 scene classification
    plus scene-event taxonomy.  Unlike /detect/events/stream (which captures
    video and runs R2Plus1D), this endpoint extracts *audio* via the camera's
    audio endpoint and classifies with CNN14.
    """
    global audio_model
...
def detect_events_endpoint():
    """
    Comprehensive event detection endpoint.
    Analyzes video for scene classification and emergency events.
    Includes geolocation from client IP.
    
    Form fields:
        video: Video file
        enable_avslowfast: Whether to use AVSlowFast (default: true)
        confidence_threshold: Event confidence threshold (default: 0.005)
    
    Returns:
        Scene classification + detected events + location
    """
    if 'video' not in request.files:
        return jsonify({'error': 'No video file provided'}), 400
...
def load_video_frames_for_event_detection(video_path, num_frames=64):
    """Load video frames as tensor for event detection"""
    try:
        cap = cv2.VideoCapture(video_path)
        if not cap.isOpened():
            return None
...
def analyze_motion_for_events(video_path, scene_class, scene_confidence, confidence_threshold=0.005):
    """
    Motion-based heuristic fallback for event detection when AVSlowFast is unavailable.

    Computes inter-frame absolute difference across a downsampled video to estimate
    activity level.  Maps average / peak motion to confidence scores for each
    scene-relevant event type.

    Calibration (downsampled to 160×120):
      avg_motion < 2.0  → static background noise → no events
      avg_motion 2–8    → mild/normal activity     → low-confidence events
      avg_motion > 8    → significant activity     → moderate-confidence events
      max_motion > 25   → sudden intense burst     → high-severity events prioritised
    """
...
def determine_event_type_from_visual(visual_result, scene_class):
    """Determine event type from visual analysis only when there is STRONG evidence
    of an actual explosion or fire — e.g. a large sudden bright flash across multiple
    frames.  Never returns a 'scene default' (like accident for bus) — that would
    inject phantom events for any video with minor brightness changes.
    Returns None when evidence is not conclusive."""
...
def detect_events_stream_endpoint():
    """
    Comprehensive event detection from IP stream.
    """
    data = request.get_json()
    if not data:
        return jsonify({'error': 'No JSON data provided'}), 400
...
def record_from_mic(duration_seconds, samplerate=44100):
    """Record audio from the default system microphone into a temp WAV file."""
...
def predict_audio_local():
    """
    Record from the local microphone and classify using CNN14.
...
def predict_video_local():
    """
    Capture from the default local camera (device index 0) and classify using
    R2Plus1D.
...
def detect_events_audio_local():
    """
    Record from the local microphone and run the full audio event detection
    pipeline (CNN14 scene classification + scene-event taxonomy).

    Request JSON:
        { "duration": 10, "confidence_threshold": 0.50 }
    """
    global audio_model
    data = request.get_json() or {}
...
def detect_events_video_local():
    """
    Capture from the local camera and run the full video event detection
    pipeline (R2Plus1D scene classification + event taxonomy + AVSlowFast).

    Request JSON:
        { "duration": 5, "confidence_threshold": 0.005,
          "enable_motion_fallback": true }
    """
    data = request.get_json() or {}
    ...
    try:
        ...
        if avslowfast_result:
            for ev_type, ev_conf in avslowfast_result.items():
                if ev_conf >= confidence_threshold:
                    if ev_type not in detected_events or ev_conf > detected_events[ev_type]:
                        detected_events[ev_type] = ev_conf
                        all_event_confidences[ev_type] = ev_conf
...
def get_geolocation_endpoint():
    """Get client's geolocation from IP"""
    client_ip = get_client_ip()
    location = get_location_from_ip(client_ip)
    return jsonify({
        'success': True,
